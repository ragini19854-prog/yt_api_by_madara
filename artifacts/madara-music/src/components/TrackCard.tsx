...
export function TrackCard({ track, onClick, showYoutubeBadge }: TrackCardProps) {
  const { play, pause, currentTrack, isPlaying } = usePlayer();
  const isCurrentTrack = currentTrack?.id === track.id;
  const isYoutube = showYoutubeBadge || track.source === "youtube";
  ...
  return (
    ...
    >
      <div className="relative aspect-square rounded-lg overflow-hidden mb-4 shadow-lg">
        ...
        {/* YouTube badge */}
        {isYoutube && (
          <div className="absolute top-2 left-2 flex items-center gap-1 bg-red-600/90 backdrop-blur-sm text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md">
